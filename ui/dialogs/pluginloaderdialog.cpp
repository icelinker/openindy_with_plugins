...
void PluginLoaderDialog::on_toolButton_Path_clicked(){
	...
	int c = (int)(100 / metaData.dependenciesPath.size());//icelinker
	int i = 0;//icelinker
    for(i = 0; i < metaData.dependenciesPath.size(); i++){
		...
		this->ui->progressBar->setValue(c*i);
    }
	if(i == metaData.dependenciesPath.size()){//icelinekr
		this->ui->progressBar->setValue(100);//ice
	}//ice
	this->ui->progressBar->setValue(100);//icelinker
